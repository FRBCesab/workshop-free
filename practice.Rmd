...
**Note** Using the `mFD` package, you can plot more than two assemblages but not with the `alpha.multidim.plot()` function.
There are several specific functions for each step of the plot: build the background of the plot (`background.plot()`),
plot the pool of species you are working on (`pool.plot()`), plot species from the studied assemblages (`species.plot()`) function and lastly plot the wanted metric using related function (`fric.plot()`, `fdiv.plot()`, `fide.plot()`,
`fdis.plot()`, `feve.plot()`, `fnnd.plot()`, `fori.plot()`, `fspe.plot()`). Plots for different axes combination can be gathered into a single plot using the `panels.to.patchwork()` function.
